...
<template>
  <section
      class="relative min-h-[calc(100vh-8rem)] w-full overflow-hidden rounded-[1.75rem] border-4 border-slate-900/20 bg-sky-200 shadow-[0_24px_60px_rgba(15,23,42,0.18)] sm:rounded-[2rem]"
  >
    <div class="absolute inset-0 bg-gradient-to-b from-sky-200 via-sky-100 to-emerald-100"></div>

    <div
        class="absolute inset-x-0 top-0 h-56 bg-repeat-x bg-top opacity-80 sm:h-64"
        style="background-image: url('/assets/background/overworld/background_clouds.png'); background-size: 320px auto;"
    ></div>

    <div
        class="absolute inset-x-0 bottom-24 h-36 bg-repeat-x bg-bottom opacity-70 sm:h-44"
        style="background-image: url('/assets/background/overworld/background_color_trees.png'); background-size: 320px auto;"
    ></div>

    <div
        class="absolute inset-x-0 bottom-0 h-28 bg-repeat-x bg-bottom sm:h-36"
        style="background-image: url('/assets/background/overworld/background_solid_grass.png'); background-size: 320px auto;"
    ></div>

    <div class="relative z-10 w-full px-3 py-4 sm:px-5 sm:py-6 lg:px-8 lg:py-8">
      <div
          class="mb-6 grid gap-4 rounded-[1.75rem] border-4 border-slate-900/15 bg-white/72 p-4 backdrop-blur-sm sm:mb-8 sm:p-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:gap-6 lg:p-8"
      >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <div class="min-w-0">
            <p class="text-xs uppercase tracking-[0.25em] text-emerald-700 sm:text-sm">
              Player Profile
            </p>
            <h1 class="break-words text-2xl text-slate-900 sm:text-3xl lg:text-4xl">
        ...
        <div
            class="justify-self-start rounded-2xl border-4 border-yellow-200 bg-yellow-100 px-4 py-3 text-sm text-yellow-900 shadow-[0_6px_0_rgba(202,138,4,0.18)] lg:justify-self-end"
        >
      ...
      <div
          v-if="loading"
          class="rounded-[1.75rem] border-4 border-sky-200 bg-white/85 px-6 py-14 text-center backdrop-blur-sm"
      >
      ...
      <div
          v-else-if="error"
          class="rounded-[1.75rem] border-4 border-red-200 bg-red-50/90 px-6 py-14 text-center backdrop-blur-sm"
      >
      ...
      <div v-else class="space-y-6 sm:space-y-8">
        <section
            class="rounded-[1.75rem] border-4 border-slate-900/15 bg-white/72 p-4 backdrop-blur-sm sm:p-6 lg:p-8"
        >
          <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="self-start rounded-2xl bg-sky-100 px-4 py-2 text-sm text-sky-800">
              Keep going
            </div>
          </div>
        ...
        >
          ...
          <div v-if="createdLevels.length > 0" class="grid grid-cols-1 gap-4 2xl:grid-cols-2">
